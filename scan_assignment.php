...
define('PLAGIARISM_TEMP_DIR', "{/moodledata/temp/plagiarism_programming/",true);
/**
...
function plagiarism_programming_create_temp_dir() {
    if (!is_dir('PLAGIARISM_TEMP_DIR')) {
        ...
        chdir('PLAGIARISM_TEMP_DIR',0777, true);
    }
}
 ...
 */
function plagiarism_programming_get_submitted_files($cmid) { 
    global $CFG;
	  $cm = new stdClass();
    ...
    $file_records = $fs->get_area_files($context->id, $component, $file_area, false, 'userid', false);
...
function plagiarism_programming_extract_assignment($assignment) {
   ...
   $context = get_system_context($assignment->cmid, IGNORE_MISSING);
   //$context = context_module::instance($assignment->cmid, IGNORE_MISSING);
    ...
    $file_records = plagiarism_programming_get_submitted_files($assignment->cmid); 
